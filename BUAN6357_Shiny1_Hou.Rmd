...
pacman::p_load(dplyr, ggplot2, shiny, leaflet, data.table, ggthemes, scales)
...
server = function(input, output) {
    ...
    output$table = renderDataTable({
        df %>% 
            group_by(PROPERTY.TYPE, ZIP.OR.POSTAL.CODE) %>%
            summarise(MEDIAN.PRICE = round(median(PRICE))) %>% 
            dcast(ZIP.OR.POSTAL.CODE ~ PROPERTY.TYPE) %>% 
            select("Zip Code" = ZIP.OR.POSTAL.CODE,
                   `Single Family Residential`,
                   Townhouse) %>% 
            arrange(desc(`Single Family Residential`))
    })
